...
namespace Pixeye.Actors
{
	public class ComponentConsole : MonoBehaviour
	{
		...
		{
			debugStr.Clear();
			debugStr.Append($"FPS: {fps.ToString("F1")} Time: {Actors.time.delta}ms GPU memory: {SystemInfo.graphicsMemorySize}mb Sys Memory: {SystemInfo.systemMemorySize}mb")
				...
				.Append($"\nActors: {Entity.GetLiveEntities()} Ticks: {ProcessorUpdate.Default.GetTicksCount()}");
